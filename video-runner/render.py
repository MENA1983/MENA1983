#!/usr/bin/env python3
"""Standalone, fail-closed visual video renderer."""
from __future__ import annotations
import argparse, hashlib, json, shutil, subprocess, sys, tempfile, urllib.parse, urllib.request
...
def run(args):
    ...
    if p.returncode:
        detail=(p.stderr or p.stdout).strip()[-3000:]
...
def https_url(value,label):
    ...
    if p.scheme!="https" or not p.netloc or p.fragment:
        fail(f"{label} must be an HTTPS URL without a fragment")
    return url

def download(url,target,max_bytes):
    req=urllib.request.Request(url,headers={"User-Agent":"external-video-runner/1"})
...
def render_scene(root,scene,index,voice):
    ...
    if audio_url:
        download(https_url(audio_url,f"scene {index} audio"),audio,MAX_AUDIO_BYTES)
    else:
        selected_voice=voice if voice in {"en","ar"} else "en"
        speed="145" if selected_voice=="ar" else "155"
        run(["espeak-ng","-v",selected_voice,"-s",speed,"-w",str(audio),narration])

    dur=max(1.0,duration(audio))
    if dur>60: fail(f"scene {index}: duration exceeds 60 seconds")
    caption=root/f"caption-{index}.txt"
    caption.write_text(narration[:1000],encoding="utf-8")

    per_image=max(1.5,dur/len(image_paths))
    clips=[]
    for n,image in enumerate(image_paths,1):
        clip=scene_dir/f"clip-{n}.mp4"
        frames=max(45,int(per_image*30))
        vf=("scale=1080:1920:force_original_aspect_ratio=increase,"
            "crop=1080:1920,"
            f"zoompan=z='min(zoom+0.0007,1.06)':d={frames}:s=1080x1920:fps=30,"
            "format=yuv420p,fade=t=in:st=0:d=0.15,"
            f"fade=t=out:st={max(per_image-0.15,0):.3f}:d=0.15")
        run(["ffmpeg","-y","-loop","1","-i",str(image),"-t",f"{per_image:.3f}",
             "-vf",vf,"-an","-c:v","libx264","-preset","veryfast","-crf","22",
             "-pix_fmt","yuv420p",str(clip)])
        clips.append(clip)

    concat=scene_dir/"images.txt"
    ...
    vf=(f"drawtext=fontfile={font}:textfile='{caption.as_posix()}':"
        "fontcolor=white:fontsize=52:line_spacing=12:x=(w-text_w)/2:y=h-430:"
        "box=1:boxcolor=black@0.58:boxborderw=28")
    run(["ffmpeg","-y","-i",str(silent),"-i",str(audio),"-vf",vf,
         "-map","0:v:0","-map","1:a:0","-t",f"{dur:.3f}","-c:v","libx264",
         "-preset","veryfast","-crf","21","-pix_fmt","yuv420p","-c:a","aac",
         "-b:a","128k","-af","loudnorm=I=-16:LRA=11:TP=-1.5","-shortest",
         "-movflags","+faststart",str(final_scene)])
    return final_scene
...
def main():
    ...
    if not isinstance(manifest,dict) or manifest.get("schema")!="external-video/v1":
        fail("unsupported manifest schema")
    scenes=manifest.get("scenes")
    if not isinstance(scenes,list) or not scenes or len(scenes)>MAX_SCENES:
        fail("scenes must contain 1..20 items")
    ...
    with tempfile.TemporaryDirectory(prefix="external-video-") as temp:
        root=Path(temp)
        clips=[render_scene(root,s,i,voice) for i,s in enumerate(scenes,1)]
